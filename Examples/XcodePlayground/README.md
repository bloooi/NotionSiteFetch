...
에러가 났다면 다른 Xcode 창이 같은 Playground/패키지를 열 상태입니다. 아래 순서대로 다시 여세요.
...
`Missing package product 'NotionSiteFetch'` 는 로컬 패키지 경로가 어깃났을 때 났고, 지금은 `Package.swift` 가 있는 저장소 루트를 가리킵니다.
